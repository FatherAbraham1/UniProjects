 ...
 */
public class SimpleSentenceFilter implements SentenceFilter {
	 ...
	 * @param verbose
	 */
	public SimpleSentenceFilter(String filterDescription, String patternString,
			int minLength, int maxLength, String hexPatternString,
			boolean verbose) {
	 ...
	 */
	public SimpleSentenceFilter(String filterDescription, String patternString,
			int minLength, int maxLength, String hexPattern) {
		this(filterDescription, patternString, minLength, maxLength,
				hexPattern, false);
	}
	...
	public boolean filterIsValid() {
		if (filterID != -1
				&& (pattern != null || hexPattern != null || minLength != 0
						|| maxLength != 0 || (replaceCharacterString != null && (replaceRatio != 0 || replaceCount != 0))))
			return true;
		else
			return false;
	}
	...
	public boolean sentenceIsValid(String sentence) {
		...
		if ((replaceCharacterString != null)
				&& (replaceRatio != 0 || replaceCount != 0)) {
			String replacedSentence = sentence;
			for (int i = 0; i < replaceCharacterString.length(); i++)
				replacedSentence = replacedSentence.replace(
						replaceCharacterString.substring(i, i + 1), "");

			if (replaceRatio != 0) {
				replaceRatioValid = ((float) sentence.length() / (replacedSentence
						.length() + 1)) <= replaceRatio;
				usedCriteria.add(!replaceRatioValid);
			} else if (replaceCount != 0) {
				replaceCountValid = sentence.length()
						- (replacedSentence.length()) < replaceCount;
		...
		if (hexPattern != null) {
			try {
				for (int i = 0; i < sentence.length(); i++) {
					String hexChar = String.format("%x", new BigInteger(1,
	...
	public SentenceFilter clone() {
		return new SimpleSentenceFilter(filterDescription, patternString,
				minLength, maxLength, hexPatternString, verbose);
	}
